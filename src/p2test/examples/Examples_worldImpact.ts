 ...
 */
class Examples_worldImpact extends egret.Sprite{
    public static inst: Examples_worldImpact;
    ...
    public constructor() {
        super();
        Examples_worldImpact.inst = this;
    ...
    private onWorldImpact(event):void{
        ...
        Examples_worldImpact.inst.onBeginContactCheck(event);
    }
    ...
    private getPlayerContactPos(): void {
        for(var i = 0;i < this.scene.world.narrowphase.contactEquations.length;i++) {
            var c: p2.ContactEquation = this.scene.world.narrowphase.contactEquations[i];
